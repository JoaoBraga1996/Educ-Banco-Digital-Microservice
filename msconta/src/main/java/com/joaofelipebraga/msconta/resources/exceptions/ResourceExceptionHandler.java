package com.joaofelipebraga.msconta.resources.exceptions;
...
import com.joaofelipebraga.msconta.services.exceptions.DataTransferObjectFoundException;
import com.joaofelipebraga.msconta.services.exceptions.DatabaseException;
...
public class ResourceExceptionHandler {
	...
	public ResponseEntity<StandardError> entityNotFound(InvalidRequestException e, HttpServletRequest request) {
		HttpStatus status = HttpStatus.BAD_REQUEST;
		StandardError err = new StandardError();
		err.setTimestamp(Instant.now());
		err.setStatus(status.value());
		err.setError("Tipo invalido");
		err.setMessage(e.getMessage());
		err.setPath(request.getRequestURI());
		return ResponseEntity.status(status).body(err);
	}
